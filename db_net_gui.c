/* $Id: db_net_gui.c,v 1.2 2003/02/22 23:17:09 d3august Exp $
*/
...
/* FIXME Allow updating the database. */
...
void addNet(GtkWidget* wi)
{
  ...
  sprintf(string, _("New network record"));
  ...
  g_signal_connect(G_OBJECT(ip_entry),   "changed",
  ...
  add_widget_to_table(GTK_WIDGET(ip_entry),   GTK_TABLE(table), 0, 1);
...
//  label = gtk_label_new(_("IP number:"));
  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(label), 0, 1, 0 ,1);
  //  gtk_widget_show(GTK_WIDGET(label));

  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(ip_entry), 1, 2, 0 ,1);
  //  gtk_widget_show(GTK_WIDGET(ip_entry));

  //  label = gtk_label_new(_("Latitude:"));
  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(label), 0, 1, 1 ,2);
  //  gtk_widget_show(GTK_WIDGET(label));

  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(lat_entry), 1, 2, 1 ,2);
  //  gtk_widget_show(GTK_WIDGET(lat_entry));

  //  label = gtk_label_new(_("Longitude:"));
  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(label), 0, 1, 2 ,3);
  //  gtk_widget_show(GTK_WIDGET(label));

  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(lon_entry), 1, 2, 2 ,3);
  //  gtk_widget_show(GTK_WIDGET(lon_entry));

  //  label = gtk_label_new(_("Info:"));
  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(label), 0, 1, 3 ,4);
  //  gtk_widget_show(GTK_WIDGET(label));

  //  gtk_table_attach_defaults(GTK_TABLE(table), GTK_WIDGET(info_entry), 1, 2, 3 ,4);
  //  gtk_widget_show(GTK_WIDGET(info_entry));
